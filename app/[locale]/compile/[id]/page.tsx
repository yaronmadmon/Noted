  ...
  if (!data || data.status === 'pending' || data.status === 'processing') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-65px)] gap-4">
        <Spinner size="lg" />
        <p className="text-gray-500 text-sm">{t('compiling')}</p>
      </div>
    )
  }
  ...
  if (data.status === 'failed' || !data.output) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-65px)] gap-3">
        ...
        <p className="text-gray-900 font-semibold">{t('failed')}</p>
        <p className="text-gray-500 text-sm">{t('failedDesc')}</p>
        <Link href="/compile" className="mt-2 text-sm text-gray-700 underline underline-offset-2">
          {t('backToCompile')}
        </Link>
      </div>
    )
    ...
    <ErrorBoundary>
      <div className="max-w-6xl mx-auto px-4 py-6 sm:py-10">
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <Link href="/compile" className="text-sm text-gray-400 hover:text-gray-600 transition-colors">
            {t('newCompilation')}
          </Link>
